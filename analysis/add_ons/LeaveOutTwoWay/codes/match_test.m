function [sigma2_psi,sigma_psi_alpha,sigma2_alpha,SE_sigma2_psi,SE_sigma_psi_alpha,SE_sigma2_alpha] = match_test(y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
%
%       - Users should be alerted that when leaving out a given match, person
...
%         setting "restrict_movers=1" and will not out results concerning
...
%         of the bounds used the function "hutchison_trace_SE"
%     
...
%Auxiliary Step
    M=sparse(1:NT,match_id',1,NT,max(match_id));
    Bii=1-diag(Lambda_P);
    NT=size(y,1);
    Pii=diag(Lambda_P);
    mm=M'*M;
...
%Sigma_i Leave Out from AKM model
    sigma_i=y.*eta_h;

%Project Match Effects on AKM model
    xy=X'*mu;
    b=pcg(xx,xy,1e-10,1000,Lchol,Lchol');
    res=mu-X*b;
    test_num=(res'*res-sum(Bii.*sigma_i))/(NT)
    stop
    res_h=pcg(I_Lambda_P,res,1e-10,1000,L_P,L_P');
    xb_mu_leave=X*b-Pii.*res_h;
    dof=NT-size(X,2)-1;
    TSS=sum((mu-mean(mu)).^2);
    R2=1-sum(res.^2)/TSS;
    adjR2=1-sum(res.^2)/TSS*(NT-1)/dof;  
    var_res=sum(res.^2)/(NT-1);
    
%Compute the Standard error now 

%Step 1: B*y=M(M'M)^(-1)(R*M)'(R*M)*(M'M)^(-1)M'*y.notice that res is 
%(R*M)*(M'M)^(-1)M'*y.
    xy=X'*res;
    b=pcg(xx,xy,1e-10,1000,Lchol,Lchol');
    res=res-X*b;
    xy=M'*res;
    b=pcg(mm,xy,1e-10,1000);
    my_first_part=M*b;


%Step 2: 0.5(Lambda_B*(I-Lambda_P)^(-1)*(I-Lambda_P)+(I-Lambda_P)*(I-Lambda_P)^(-1)Lambda_B)*y
    xy=Lambda_B*y;
    [ydelta_xi, flag]=pcg(I_Lambda_P,xy,1e-10,1000,L_P,L_P');
    xy=X'*ydelta_xi;
    [b, flag]=pcg(xx,xy,1e-10,1000,Lchol,Lchol');
    xi=ydelta_xi-X*b;
    my_second_part=0.5*(Lambda_B*eta_h+xi); 
    W_to_use=my_first_part-my_second_part;

%Step 3: perform the non-parametric fit.
    tic
    hBest=1/NT^(1/3);
...
%Everything ready to compute denominator
    NSIM=10;
    ...
    V=(1/NT^2)*(left_part); %%Imposing the null. See email on July 8th 2018.
    test_den=sqrt(V);
    
    %Finish
    test=  test_num / test_den;
