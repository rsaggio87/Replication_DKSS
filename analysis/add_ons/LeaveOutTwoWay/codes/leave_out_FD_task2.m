function [sigma2_psi,V] = leave_out_FD_task2(y,id,firmid,leave_out_level,controls,type_algorithm,eigen_diagno,eigen_fast,do_montecarlo,restrict_movers,filename)
...
SE_CONSERVATIVE=sqrt((1/NT^2)*4*sum(W_to_use.*W_to_use.*sigma_i));
%% STEP 7: REPORTING
...
disp(s)
s=['Leave One Out SE Conservative: ' num2str(SE_CONSERVATIVE)];
disp(s)
end
s=['-*-*-*-*-*-*-*-*-*-*-*-*'];
disp(s)
...
sel=sigma_predict<0;
facc=0.10*var(ydelta);
AAlpha=2;
BBeta=5;
MMM=AAlpha/(AAlpha+BBeta);
MMM=1/MMM;
%sigma_predict = facc*MMM*betarnd(AAlpha,BBeta,size(Fdelta,1),1);
sigma_predict=abs(sigma_i);
...
A_b=F'*(fe-mean(fe));
[W_to_use, my_first_part] = construc_W_FD(ydelta,Fdelta,L,pfun_,A_b,Lambda_B,I_Lambda_P,L_P,eta_h);
[sigma2_psi_s, ~, ~, COV_R1, gamma_sq,~,b_1,theta_1]= leave_out_estimation_two_way_FD(ydelta,Fdelta,F,L,pfun_,sigma_2_psi_AKM,Lambda_B,Lambda_P,I_Lambda_P,L_P,eta_h,W_to_use,my_first_part,x1bar,lambda_1);   
sigma_i=ydelta.*eta_h;
V_exact_s=((1/NT^2)*4*sum(W_to_use.*W_to_use.*sigma_i));
%ANDREWS
MSE=sum(r.^2)/dof;
...
s=['-*-*-*-*-*-*-*-*-*-*-*-*'];
disp(s)
s=['Expected Value of standard error estimator (exact when q=0): ' num2str(mean(V_exact_simul(V_exact_simul>0)))];
disp(s);
s=['-*-*-*-*-*-*-*-*-*-*-*-*'];
disp(s)
s=['Coverage Rate of oracle estimator 95%: ' num2str(mean(oracle_coverage))];
disp(s);
s=['Coverage Rate of Proposed Bounds for inference (exact) 95%: ' num2str(mean(coverage_exact_95(V_exact_simul>0)))];
disp(s);
