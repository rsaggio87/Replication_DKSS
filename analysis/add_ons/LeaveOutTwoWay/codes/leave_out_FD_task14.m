function [EIG1 EIG2 EIG3 sigma2_true theta stdTheta V_sym_SIM V_nosym_SIM sdV_sym_SIM sdV_nosym_SIM oracle_coverage coverage_SYM coverage_NOSYM]  = leave_out_FD_task14(y,id,firmid,leave_out_level,leave_2_out,controls,type_algorithm,eigen_diagno,eigen_fast,do_montecarlo,restrict_movers,filename)
...
sigma_i_or	= sigma_i;
sigma_i		= fun(b,xdata);
Xre			= [ones(Ndelta,1) sigma_i];
coo			= Xre\sigma_i_or
histogram(sigma_i(xdata(:,2)>0))
saveas(gcf,[ filename 'HISTOGRAM.png'])
...
index_circle1	= parallel.pool.Constant(index_circle1);
index_circle2	= parallel.pool.Constant(index_circle2);
focus_circle1_i	= parallel.pool.Constant(focus_circle1_i);
focus_circle1_j	= parallel.pool.Constant(focus_circle1_j);
focus_circle2_i	= parallel.pool.Constant(focus_circle2_i);
focus_circle2_j	= parallel.pool.Constant(focus_circle2_j);
PGIGA_CIRCLE1	= parallel.pool.Constant(PGIGA_CIRCLE1);
PGIGA_CIRCLE2	= parallel.pool.Constant(PGIGA_CIRCLE2);

parfor s=1:NSIM
	%ydelta						  = Fdelta*psi_true+sqrt(sigma_i).*randn(Ndelta,1);	
	ydelta						  = Fdelta*psi_true+sqrt(mean(sigma_i)).*randn(Ndelta,1);	
	R2(s)						  = var(Fdelta*psi_true)/var(ydelta);
	vec 						  = C_build_FD(ydelta,Fdelta,L,pfun_,Lambda_B,I_Lambda_P,L_P,F);
	...
	%V_sym  					  = paths_network_v3(id_movers,firmid_delta,firmid_delta_f,ydelta,vec,Fdelta,L,F,tol,epsilon,type_algorithm,pfun_,Lambda_P,Lambda_B);
	V_sym 					 	  = MC_fixed_FD_v3(ydelta,Fdelta,L,F,tol,epsilon,type_algorithm,pfun_,Lambda_P,Lambda_B,Cij.Value,D.Value,vec,id_movers(movers_sel),movers_sel,index_movers.Value,index_movers_i.Value,index_movers_2.Value,index_movers_i_2.Value,Ppath_1.Value,Ppath_2.Value,j_in_Ai1.Value,j_in_Ai2.Value,i_in_Aj1.Value,i_in_Aj2.Value,no_A2.Value,magical_number.Value,prob_pairs.Value,index_circle1.Value,index_circle2.Value,focus_circle1_i.Value,focus_circle2_i.Value,focus_circle1_j.Value,focus_circle2_j.Value,PGIGA_CIRCLE1.Value,PGIGA_CIRCLE2.Value);
