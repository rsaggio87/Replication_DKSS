function KSS_test_sim = OD_FD_sim(y,id,firmid,leave_out_level,controls,type_algorithm,eigen_diagno,eigen_fast,do_montecarlo,double_movers,filename)
...
aux=mm\M';
tic
Bii_P=zeros(Nmovers,1);
Bii_1=zeros(Nmovers,1);
parfor i=1:Nmovers
mi=M*aux(:,i);
xy=Fdelta'*mi;
...
%% STEP 9: Simulations
%Generate the deviation from the null
NSIM=1000;
psi_o=psi(firmid_delta);
psi_d=psi(firmid_delta_f);
omega=0.10;
phi=0.20*randn(max(match_OD),1);
phi=M*phi;
MSE=sum((ydelta-mu).^2)/(Ndelta-size(M,2)-1);
%Fill up vectors);
KSS_test_sim=zeros(NSIM,1);
for ss=1:NSIM
    ydelta=phi+randn(Ndelta,1)*sqrt(MSE);
    %% AKM
    xy=Fdelta'*ydelta;
    [psi,flag]=pcg(L,xy,1e-10,1000,pfun_);
    eta=ydelta-Fdelta*psi;
    ...
    V=(var(aux_SIM))/Ndelta^2;
    test_num/sqrt(V)
    KSS_test_sim(ss)=test_num/sqrt(V);

end
save([filename 'OD_SIM'])
