function [R2,adjR2] = leave_out_FD_task3(QQQ,y,id,firmid,leave_out_level,controls,type_algorithm,eigen_diagno,eigen_fast,do_montecarlo,restrict_movers,filename)
...
xb=Fdelta*psi_hat_norm;
eta=ydelta-xb;
dof=size(ydelta,1)-size(Fdelta,2)-1;
TSS=sum((ydelta-mean(ydelta)).^2);
R2=1-sum(eta.^2)/TSS
adjR2=1-sum(eta.^2)/TSS*(size(ydelta,1)-1)/dof
...
if QQQ == 50
       ...
       lambda=1./lambda;
       out=[lambda]
       dlmwrite('EIGEN_DELTA.csv', out, 'delimiter', '\t', 'precision', 16);
end
Xsmall=[ones(size(Fdelta,1),1) Fdelta*V];
b=Xsmall\ydelta;
xb=Xsmall*b;
eta=ydelta-xb;
dof=size(ydelta,1)-size(Xsmall,2)-1;
TSS=sum((ydelta-mean(ydelta)).^2);
R2=1-sum(eta.^2)/TSS
adjR2=1-sum(eta.^2)/TSS*(size(ydelta,1)-1)/dof
