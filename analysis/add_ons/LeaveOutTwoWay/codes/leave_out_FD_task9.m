function NUMBER = leave_out_FD_task9(y,id,firmid,leave_out_level,controls,type_algorithm,eigen_diagno,eigen_fast,do_montecarlo,restrict_movers,filename)
...
list			= [firmid_delta firmid_delta_f (1:size(firmid_delta,1))'];
list_aux 		= [firmid_delta_f firmid_delta (1:size(firmid_delta,1))'];
sel   			= firmid_delta~=firmid_delta_f;
list 			= list(sel,:);
...
[~,~,edge_index]= unique(list(:,1:2),'rows','stable');
summary			= accumarray(edge_index,1);
single_edge 	= summary==1;
mean(single_edge) 
single_edge = single_edge(edge_index);
mean(single_edge) %p-y weighting the moves
indicator=sparse(list(:,3),1,single_edge,size(firmid_delta,1),1);

Bii=diag(Lambda_B);
SUM = sum(Bii)
NUMBER = sum(Bii(indicator))/SUM
