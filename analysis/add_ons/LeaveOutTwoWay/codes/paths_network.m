function  [V_sym V_nosym Q_share C_share_1 C_share_2 C_share_3 C_share_4 C_share_5 Chamard Dsym vec index_movers index_movers_i index_movers_2 index_movers_i_2 Ppath_1 Ppath_2 no_A2 list_problem taxonomy magical_number TAXONOMY_I TAXONOMY_J MOVERS_i MOVERS_j FOCUS_I FOCUS_J Ppath_PROBLEM_i Ppath_PROBLEM_j taxonomyALT TAXONOMY_ALT_I TAXONOMY_ALT_J MOVERS_ALT_i MOVERS_ALT_j FOCUS_ALT_I FOCUS_ALT_J Ppath_ALT_PROBLEM_i Ppath_ALT_PROBLEM_j] 	 = paths_network(id_movers,firmid_delta,firmid_delta_f,ydelta,C)
...
problem1=cell(Nmovers,1);
problem2=cell(Nmovers,1);
problem3=cell(Nmovers,1);
...
parfor i = 1:Nmovers			%nota: questo parfor puo' essere chiaramente evitato con uno uso intellingente di meshgrid 
	   ...
	   problem{i}			= [problem1(sel) problem2(sel) problem3(sel)];
end
...
magical_number 	 = (C(sub2ind(size(C),list_problem(:,1),list_problem(:,2)))).^2 +  2*(D(sub2ind(size(D),list_problem(:,1),list_problem(:,2)))) + 2*(D(sub2ind(size(D),list_problem(:,2),list_problem(:,1))))  ;
