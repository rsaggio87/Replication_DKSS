function [sigma2_psi_simul, V_simul, coverage_simul] = cluster_match_MC(y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
%		at unique matches.
%
% 2.1:  Fine-tune some parts to facilitate integration with "lincom_KSS"
...
%         This information is used to conduct the smoothing step, see
%         function "smooth_fn" for details.
...
%high rank case.
    if do_SE  == 1  
        ...
        iter_bw = 1;
        sigma_predict= smooth_fn(elist,Lambda_P,Lambda_B,year,y,eta_h,movers,K,T,subsample_llr_fit,leave_out_level,restrict_movers,type_quadratic_form,iter_bw);
    end
...
%Run
    ...
    if eigen_diagno == 0  && do_SE == 1
        [theta(pp), V_theta(pp),~, ~, ~, ~, ~,FIRST]= leave_out_estimation_two_way(type_quadratic_form,y,J,N,X,xx,Lchol,bias_part,eta_h,I_Lambda_P,L_P,Lambda_B,W_to_use,my_first_part,sigma_predict);
        NITER_Bw=10;
        iter_bw=2;
        CONDi = V_theta(pp)>0;
        
        while CONDi ~= 1 
            disp('Obtained an imaginary SE. Doubling the b/w in smoothing step')    
            
            %Recalculate the \tilde \sigma_i for higher bw.
            sigma_predict = smooth_fn(elist,Lambda_P,Lambda_B,year,y,eta_h,movers,K,T,subsample_llr_fit,leave_out_level,restrict_movers,type_quadratic_form,iter_bw);
            ...
            if iter_bw >= NITER_Bw
                CONDi=1;
                V_theta(pp)=FIRST;   
                disp('Could not obtain a real value SE after 10 iterations. Try increasing NITER_Bw from 10 to something more in "Leave out complete". Reporting first component of the SE in the high rank case (conservative)')    
            end
            
        end
    end
    if eigen_diagno == 1  && do_SE == 1
         [theta(pp), V_theta(pp), COV_R1(:,:,pp), gamma_sq(pp), F_stat(pp), b_1(pp), theta_1(pp),FIRST]= leave_out_estimation_two_way(type_quadratic_form,y,J,N,X,xx,Lchol,bias_part,eta_h,I_Lambda_P,L_P,Lambda_B,W_to_use,my_first_part,sigma_predict,x1_bar,lambda_1(pp));
         NITER_Bw=10;
         iter_bw=2;
         CONDi = V_theta(pp)>0;
        
        while CONDi ~= 1 
            disp('Obtained an imaginary SE. Increasing bandwidth in smoothing step')    
            
            %Recalculate the \tilde \sigma_i for higher bw.
            sigma_predict = smooth_fn(elist,Lambda_P,Lambda_B,year,y,eta_h,movers,K,T,subsample_llr_fit,leave_out_level,restrict_movers,type_quadratic_form,W_to_use,2);
            [theta(pp), V_theta(pp), COV_R1(:,:,pp), gamma_sq(pp), F_stat(pp), b_1(pp), theta_1(pp),FIRST]= leave_out_estimation_two_way(type_quadratic_form,y,J,N,X,xx,Lchol,bias_part,eta_h,I_Lambda_P,L_P,Lambda_B,W_to_use,my_first_part,sigma_predict);
            ...
            if iter_bw >= NITER_Bw
                CONDi=1;
                V_theta(pp)=FIRST;   
                disp('Could not obtain a real value SE after 10 iterations. Try increasing NITER_Bw in "Leave out complete". Reporting first component of the SE in the high rank case (conservative)')    
            end
...
%%%Description
%----------------------------------------------------
% Montecarlo study for leave out of variance components
% with serial correlation within matches. 
%----------------------------------------------------


%% PART 1.1: VCM OF THE ERROR TERM (serial correlation within cluster, no stationary)
MSE = sum(eta.^2)/(NT-size(xx,2)-1)
[~,~,clustering_var]=unique(clustering_var);
M=max(clustering_var);
MM  = sparse((1:NT)',clustering_var,1,NT,M);
factor1=0.10*var(y(year<=1998))
factor2=0.20*var(y(year>1998))

%% PART 2: SET-UP DGP
NSIM=1000;

psi_true=ghat*sqrt(sigma2_psi/sigma_2_psi_AKM);
sigma2_true=var(F*S*psi_true)

sigma2_psi_simul=zeros(NSIM,1);
V_simul=zeros(NSIM,1);
coverage_simul=zeros(NSIM,1);
flag_1=zeros(NSIM,1);
flag_2=zeros(NSIM,1);
...
pp=1;
descrMethod='KSS-Leave Out on Matches'
subsample_llr_fit=1;
[~,~,yy]=unique(year);
controls=sparse(1:(size(y,1)),yy,1,size(y,1),max(yy));
W=[X controls];
ww=W'*W;
Wchol=ichol(ww,struct('type','ict','droptol',1e-2,'diagcomp',.1));
for s=1:NSIM
      %DGP
      y=pe+F*S*psi_true+sqrt(factor1)*(year<=1998).*MM*randn(M,1)+sqrt(factor2)*(year>1998).*MM*randn(M,1)+sqrt(MSE)*randn(NT,1);
      
      %Resid
      xy=W'*y;
      b=pcg(ww,xy,1e-10,1000,Wchol,Wchol');
      y=y-W(:,N+J:end)*b(N+J:end);
      ...
      iter_bw = 1;
      sigma_predict= smooth_fn(elist,Lambda_P,Lambda_B,year,y,eta_h,movers,K,T,subsample_llr_fit,leave_out_level,restrict_movers,type_quadratic_form,iter_bw);
      [sigma2_psi_simul(s,pp),V_simul(s,pp),~,~,~,~,~,FIRST]= leave_out_estimation_two_way('fe',y,J,N,X,xx,Lchol,sigma_2_psi_AKM,eta_h,I_Lambda_P,L_P,Lambda_B,W_to_use,my_first_part,sigma_predict);
      
     %CHECKING SE
      NITER_Bw = 10;
      iter_bw  = 2;
      CONDi = V_simul(s,pp)>0;
      while CONDi ~=1
                flag_1(s)=1;
                sigma_predict= smooth_fn(elist,Lambda_P,Lambda_B,year,y,eta_h,movers,K,T,subsample_llr_fit,leave_out_level,restrict_movers,type_quadratic_form,iter_bw);
                [sigma2_psi_simul(s,pp),V_simul(s,pp)]= leave_out_estimation_two_way('fe',y,J,N,X,xx,Lchol,sigma_2_psi_AKM,eta_h,I_Lambda_P,L_P,Lambda_B,W_to_use,my_first_part,sigma_predict);
                CONDi = V_simul(s,pp)>0;
                iter_bw=iter_bw+1;
                if iter_bw >= NITER_Bw
                    CONDi=1;
      ...
      V_simul(s,pp)=sqrt(V_simul(s,pp));
      UB=sigma2_psi_simul(s,pp)+1.96*V_simul(s,pp);
      LB=sigma2_psi_simul(s,pp)-1.96*V_simul(s,pp);
      coverage_simul(s,pp)=(sigma2_true>=LB && sigma2_true<= UB);
      disp('Completed simulation #')
      s
      save('MC_cluster_match_iter')
end      
      save('MC_cluster_match_new')
      %Oracle Coverage
      UB=sigma2_psi_simul(:,1)+1.96*std(sigma2_psi_simul(:,1));
      LB=sigma2_psi_simul(:,1)-1.96*std(sigma2_psi_simul(:,1));
      coverage_oracle=(sigma2_true>=LB & sigma2_true<= UB);
...
%% PART 4: REPORT
    s=['******************************************'];
    disp(s); 
    s=['Mean Estimator of Variance of Firm Effects' num2str(sigma2_true)];
    disp(s)
    s=['Oracle Coverage' num2str(mean(coverage_oracle))];
    disp(s)
    s=['******************************************'];
...
for pp = 1 : 1
    s=['******************************************'];
    disp(s); 
    s=['Mean Estimator of  Variance of Firm Effects' descrMethod '  ' num2str(mean(sigma2_psi_simul(:,pp)))];
    disp(s)
    s=['Std Estimator of   Variance of Firm Effects' descrMethod '  ' num2str(std(sigma2_psi_simul(:,pp)))];
    disp(s)
    s=['Mean Std Error of  Variance of Firm Effects ' descrMethod '  ' num2str(mean(V_simul(:,pp)))];
    disp(s)
    s=['Mean Coverage' descrMethod '  ' num2str(mean(coverage_simul(:,pp)))];
    disp(s)
    s=['Fraction of cases where KSS did not return a positive SE' '  ' num2str(mean(flag_2))];
    disp(s)
    s=['Fraction of cases where KSS did not return a positive SE with small bw'  '  ' num2str(mean(flag_1))];
    disp(s)
    s=['******************************************'];
