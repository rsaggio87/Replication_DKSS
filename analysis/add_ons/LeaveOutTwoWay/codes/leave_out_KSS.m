function [sigma2_psi,sigma_psi_alpha,sigma2_alpha,SE_sigma2_psi,SE_sigma_psi_alpha,SE_sigma2_alpha] = leave_out_COMPLETE(y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,scale,filename)
...
%                    Default is "exact".
%                
%                 2. scale: this governs the tradeoff b/w speed and unbiasdness 
%                    when estimating (Bii,Pii). Smaller values of scale implies 
...
%-%-%- %-%-%- %-%-%- %-%-%- %-%-%- %-%-%- %-%-%- %-%-%- %-%-%- %-%-%- 
%scale: This should be b/w 0 and 1.

%scale governs the tradeoff b/w speed and accuracy when estimating 
%(Bii,Pii) using type_of_algorithm=JLL.

%Smaller values of scale implies less accuracy but higher computation time.
...
disp('time to construct indexes needed to speed-up computation of Pii')
toc
...
clear n lagfirmid sel stayer gcs D F
tic
...
if n_of_parameters==1
    [Lambda_P, Lambda_B_fe]=eff_res_KSS(X,xx,Lchol,N,J,K,elist,leave_out_level,movers,T,type_of_algorithm,id,firmid,match_id,scale);
end

if n_of_parameters==2
    [Lambda_P, Lambda_B_fe,Lambda_B_cov]=eff_res_KSS(X,xx,Lchol,N,J,K,elist,leave_out_level,movers,T,type_of_algorithm,id,firmid,match_id,scale);
end

if n_of_parameters==3
    [Lambda_P, Lambda_B_fe,Lambda_B_cov,Lambda_B_pe]=eff_res_KSS(X,xx,Lchol,N,J,K,elist,leave_out_level,movers,T,type_of_algorithm,id,firmid,match_id,scale);
end

if strcmp(leave_out_level,'obs') & strcmp(type_of_algorithm,'JLL') 
[bound_1 bound2] = quality_JLL(Lambda_P,Lambda_B_fe,y,xx,X,Lchol,scale)
end
...
if eigen_diagno==1 
qKSS=2;
EIG_NORM=zeros(3,qKSS,n_of_parameters);
max_x1bar_sq=zeros(qKSS,n_of_parameters,1);
lambda_1=zeros(qKSS,n_of_parameters,1);
SUM_EIG=zeros(n_of_parameters,1);
x1bar_all=zeros(NT,qKSS,n_of_parameters);
tic    
...
for pp=1:n_of_parameters
    ...
    %Calculate
    disp(entry)
    [Q,lambda_eig] = eigAux(type_quadratic_form,xx,Lchol,F*S,D,K,qKSS);
    lambda_eig=diag(lambda_eig);
    lambda_1(:,pp)=lambda_eig(1:qKSS); 
...
clear A_b Lambda_B I_Lambda_P L_P
...
%out=[y,firmid,id,firmid_old,id_old,Pii,full(controls)];
%s=[filename '.csv'];
