function [sigma2_psi,sigma_psi_alpha,sigma2_alpha,SE_sigma2_psi,SE_sigma_psi_alpha,SE_sigma2_alpha] = leave_out_COMPLETE_pls(QQQ,y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
%high rank case.
    ...
    if do_SE  == 2  
        rows=elist(:,1);
        columns=elist(:,2);
        if pp == 1 %RUN PLS only once
            [~,yl,XS,~,~,~]= plsregress(X,y,QQQ);
            eta_pls=y-XS*yl';
            eta_pls=eta_pls-mean(eta_pls);
            sigma_predict = 0.5*(eta_pls(rows).*eta_pls(columns)+eta_pls(columns).*eta_pls(rows));
            sigma_predict = sparse(rows,columns,sigma_predict,NT,NT);
        end
        eta_h=eta_pls;
    end
    if do_SE ~= 2
    error('non stai a fa la cosa giusta')    
    end    
...
%Run
    ...
    if eigen_diagno == 0  && do_SE >= 1
        [theta(pp), V_theta(pp)]= leave_out_estimation_two_way_pls(type_quadratic_form,y,J,N,X,xx,Lchol,bias_part,eta_h,I_Lambda_P,L_P,Lambda_B,W_to_use,my_first_part,sigma_predict);
        NITER_Bw=10;
        iter_bw=2;
        CONDi = V_theta(pp)>0;
        
        while CONDi ~= 1 && do_SE == 1
