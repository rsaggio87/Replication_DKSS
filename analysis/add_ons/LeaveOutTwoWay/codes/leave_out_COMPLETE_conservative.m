function [sigma2_psi,sigma_psi_alpha,sigma2_alpha,SE_sigma2_psi,SE_sigma_psi_alpha,SE_sigma2_alpha] = leave_out_COMPLETE_conservative(y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
%high rank case.
    ...
    if do_SE  == 2  
        ...
        columns=elist(:,2);
        if pp == 1
            %sigma_predict = 0.5*(eta_h(rows).*eta_h(columns)+eta_h(columns).*eta_h(rows)); %HC3
            sigma_predict = 0.5*(y(rows).*eta_h(columns)+eta_h(columns).*y(rows)); %KSS
            sigma_predict = sparse(rows,columns,sigma_predict,NT,NT);
        end    
        SUM=sum(W_to_use'*sigma_predict*W_to_use); 
        V_theta(pp)=(1/NT^2)*(4*SUM); 
    end

%Run
    if eigen_diagno == 1  && do_SE == 0 || eigen_diagno == 1  && do_SE == 2 
        [theta(pp)]= leave_out_estimation_two_way(type_quadratic_form,y,J,N,X,xx,Lchol,bias_part,eta_h,I_Lambda_P,L_P,Lambda_B);
    end

    if eigen_diagno == 0  && do_SE == 0 || eigen_diagno == 0  && do_SE == 2
        [theta(pp)]= leave_out_estimation_two_way(type_quadratic_form,y,J,N,X,xx,Lchol,bias_part,eta_h,I_Lambda_P,L_P,Lambda_B);
    end
