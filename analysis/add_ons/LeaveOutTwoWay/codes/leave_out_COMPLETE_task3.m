function [R2,adjR2] = leave_out_COMPLETE_task3(QQQ,y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
[V,~]=eigs(xx,QQQ);
Xsmall=[ones(NT,1) X*V];
b=Xsmall\y;
xb=Xsmall*b;
eta=y-xb;
dof=NT-size(Xsmall,2)-1;
TSS=sum((y-mean(y)).^2);
R2=1-sum(eta.^2)/TSS
adjR2=1-sum(eta.^2)/TSS*(NT-1)/dof
