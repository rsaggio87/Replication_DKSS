function R2 = leave_out_COMPLETE_task5(QQQ,y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
S=[S;sparse(-zeros(1,J-1))];  %N+JxN+J-1 restriction matrix     
X=[D,F*S];
[~,yl,XS,~,~,PCTVAR]= plsregress(X,y,QQQ);
xb=XS*yl';
R2=corr(y,xb)^2
plot(1:QQQ,cumsum(100*PCTVAR(2,:)),'-bo');
xlabel('Number of PLS components: M');
ylabel('R2');
print([filename '_pls'],'-dpng')
end
