function [Lambda_P, Lambda_B_fe, Lambda_B_cov, Lambda_B_pe] = eff_res_KSS(X,xx,Lchol,N,J,K,elist,clustering_level,movers,T,type_algorithm,id,firmid,match_id,scale,tolProb)
...
if K == 0
    ...
    M=size(elist_JLL,1);
    Pii_movers=sparse(M,1);
    Bii_fe_movers=sparse(M,1);
    Bii_cov_movers=sparse(M,1);
    Bii_pe_movers=sparse(M,1);
    clear count 
end
...
if K==0
   ...
   if  strcmp(type_algorithm,'JLL') 
       ...
       LCHOL = parallel.pool.Constant(@() Lchol);
       ...
       parfor i=1:scale
                ...
                %Lambda_P
                [Z, flag]= pcg(xx_c.Value,(ons*(X_c.Value))',tol,numIterations,LCHOL.Value);
                
                %Lambda_B_fe
                ons=ons-mean(ons);
                [ZB, flag]= pcg(xx_c.Value,(ons*X_fe.Value)',tol,numIterations,LCHOL.Value);
                
                %Lambda_B_cov
                if do_cov == 1
                    [ZB_pe, flag]= pcg(xx_c.Value,(ons*X_pe.Value)',tol,numIterations,LCHOL.Value);
                end
...
sel = Pii>0.99;
disp('the following share of obs have a Pii above 0.99')
mean(sel)
if sum(sel)>0
disp(' ---- WINSORIZING THESE OBS -- WE SUGGEST TO INCREASE THE SCALE OF THE JLL ALGORITHM TO IMPROVE STABILITY')
Pii(sel)=0.99;
end
