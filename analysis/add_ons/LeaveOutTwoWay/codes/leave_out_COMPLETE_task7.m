function [sigma2_psi,sigma_psi_alpha,sigma2_alpha,SE_sigma2_psi,SE_sigma_psi_alpha,SE_sigma2_alpha] = leave_out_COMPLETE_task7(y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
%Run
    ...
    if eigen_diagno == 0  && do_SE == 1
        ...
        Bii = diag(Lambda_B);
        SI	= diag(sigma_predict);
        prova = bias_part - (sum(Bii.*SI))*(1/NT)
        NITER_Bw=10;
        iter_bw=2;
        CONDi = V_theta(pp)>0;
