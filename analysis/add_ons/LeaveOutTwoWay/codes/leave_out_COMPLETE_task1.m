function [sigma2_psi,sigma_psi_alpha,sigma2_alpha,SE_sigma2_psi,SE_sigma_psi_alpha,SE_sigma2_alpha] = leave_out_COMPLETE_task1(y,id,firmid,leave_out_level,year,controls,resid_controls,andrews_estimates,eigen_diagno,subsample_llr_fit,restrict_movers,do_SE, type_of_algorithm,epsilon,filename)
...
%high rank case.
    if do_SE  == 1  
        ...
        columns=elist(:,2);
        if pp == 1 %only once
            sigma_predict = 0.5*(y(rows).*eta_h(columns)+eta_h(columns).*y(rows));
    ...
    if do_SE  == 2  
        ...
        columns=elist(:,2);
        if pp == 1
            sigma_predict = 0.5*(eta_h(rows).*eta_h(columns)+eta_h(columns).*eta_h(rows)); %compute HC3 only once.
            sigma_predict = sparse(rows,columns,sigma_predict,NT,NT);
        end    
